...
function DesignerApp() {
  ...
  const buildDesigner = useCallback(async () => {
    ...
    try {
      ...
      // Add exportPdf method to the designer - allows generating the complete PDF
      designer.current.exportPdf = async () => {
        const { generate } = await import('@pdfme/generator');
        const template = designer.current?.getTemplate() ?? getBlankTemplate();
        const options = designer.current?.getOptions() ?? {};
        const font = getFontsData();

        const pdf = await generate({
          template,
          inputs: [{}], // Empty inputs for template with placeholders
          options: {
            font,
            lang: options.lang,
            title: 'pdfme-export',
  ...
  const navItems: NavItem[] = [
    ...
    {
      label: 'PDF Manipulator',
      content: (
        <button
          className={`px-2 py-1 border rounded hover:bg-gray-100 w-full disabled:opacity-50 disabled:cursor-not-allowed`}
          onClick={() => setShowManipulator(!showManipulator)}
        >
          {showManipulator ? 'Hide Manipulator' : 'Show Manipulator'}
        </button>
      ),
    },
    {
      ...
      content: (
        <div className="flex gap-2">
          ...
          <button
            id="generate-pdf"
            disabled={editingStaticSchemas}
            className={`px-2 py-1 border rounded hover:bg-blue-500 hover:text-white bg-blue-50 w-full ${
              editingStaticSchemas ? 'opacity-50 cursor-not-allowed' : ''
            }`}
            onClick={async () => {
