...
import { pdf2img } from '@pdfme/converter'; // For generating thumbnails
...
// Helper to generate thumbnails from PDF
const generateThumbnails = async (pdfData: ArrayBuffer | Uint8Array | string): Promise<string[]> => {
  try {
    // Skip thumbnail generation for BLANK_PDF
    if (pdfData === BLANK_PDF) {
      return [];
    }
    ...
    if (typeof pdfData === 'string') {
      if (pdfData.startsWith('data:application/pdf;base64,')) {
        ...
        pdfBuffer = new Uint8Array(byteString.length);
        for (let i = 0; i < byteString.length; i++) {
          (pdfBuffer as Uint8Array)[i] = byteString.charCodeAt(i);
        }
      } else {
        console.warn('PDF data is not in expected base64 format');
        return [];
      }
    } else {
      pdfBuffer = pdfData;
    ...
    // Generate images at a small scale for thumbnails
    const thumbnailImages = await pdf2img(pdfBuffer, {
      scale: 0.2,  // Small scale for thumbnails
      imageType: 'png'
    });
    
    // Convert ArrayBuffer images to base64 strings for <img> tags
    return thumbnailImages.map(imgBuffer => {
      try {
        const binary = new Uint8Array(imgBuffer);
        let binaryStr = '';
    ...
    }).filter(Boolean); // Remove any empty strings from failed conversions
  } catch (error) {
...
export function MultiPageTemplateEditor({
    ...
    try {
      ...
      if (newBasePdf === BLANK_PDF) {
        ...
        if (typeof newBasePdf === 'string' || newBasePdf instanceof Uint8Array) {
          ...
          // Generate thumbnails for the updated PDF
          const thumbnails = await generateThumbnails(newBasePdf);
          
          // Update the thumbnails in pagesMeta
          if (thumbnails.length > 0) {
            // Update thumbnails for existing pages
            newPagesMeta.forEach((pageMeta, idx) => {
              if (idx < thumbnails.length - 1) { // -1 because last thumbnail is for the new page
                pageMeta.thumbnailSrc = thumbnails[idx];
              }
            });
            
            // Add new page meta with thumbnail
            newPagesMeta.push({
              ...createInitialPageMeta(newPagesMeta.length),
              thumbnailSrc: thumbnails[thumbnails.length - 1] // Last thumbnail is for the new page
            });
          } else {
            // Add new page meta without thumbnail
            newPagesMeta.push(createInitialPageMeta(newPagesMeta.length));
          }
  ...
  const handleAddPdfFiles = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        // Track starting number of pages
        const _ = currentPdfDoc.getPageCount();
        ...
        const thumbnails = await generateThumbnails(mergedPdfBytes);
        
        // Apply thumbnails to the page metadata
        if (thumbnails.length > 0) {
          // Update thumbnails for all pages
          for (let i = 0; i < currentPagesMeta.length; i++) {
            if (i < thumbnails.length) {
              currentPagesMeta[i].thumbnailSrc = thumbnails[i];
            }
          }
        }
        ...
        if (newBasePdf !== BLANK_PDF) {
          if (typeof newBasePdf === 'string') {
            ...
            if (pageIndex < pdfDoc.getPageCount()) {
              ...
              // Regenerate thumbnails since the PDF structure has changed
              const thumbnails = await generateThumbnails(newBasePdf);
              
              // Update thumbnails for remaining pages
              if (thumbnails.length > 0) {
                newPagesMeta.forEach((pageMeta: PageMeta, i: number) => {
                  if (i < thumbnails.length) {
                    pageMeta.thumbnailSrc = thumbnails[i];
                  }
                });
              }
            } else {
    ...
    async (fromIndex: number, toIndex: number) => {
      ...
      try {
        ...
        if (template.basePdf !== BLANK_PDF) {
          ...
          // Regenerate thumbnails since PDF page order has changed
          const thumbnails = await generateThumbnails(finalBasePdf);
          
          // Update thumbnails to match new page order
          if (thumbnails.length > 0) {
            newPagesMeta.forEach((pageMeta: PageMeta, i: number) => {
              if (i < thumbnails.length) {
                pageMeta.thumbnailSrc = thumbnails[i];
  ...
  const handleGenerateFinalPDF = useCallback(async () => {
    ...
    try {
      ...
      if (template.basePdf instanceof Uint8Array) {
        ...
        const btoaSafeString = arr.map((byte: number) => String.fromCharCode(byte)).join('');
      ...
      // Create a proper blob from the PDF buffer
      const pdfBytes = pdf.buffer instanceof ArrayBuffer 
        ? new Uint8Array(pdf.buffer) 
        : new Uint8Array(pdf.buffer.byteLength).set(new Uint8Array(pdf.buffer));
  ...
  const handleLoadUserTemplate = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        if (newTemplate.basePdf !== BLANK_PDF) {
          const thumbnails = await generateThumbnails(newTemplate.basePdf);
          
          // Apply thumbnails to page metadata
          if (thumbnails.length > 0) {
            for (let i = 0; i < newPagesMeta.length; i++) {
              if (i < thumbnails.length) {
                newPagesMeta[i].thumbnailSrc = thumbnails[i];
              }
            }
          }
        }
  ...
  const ensurePageContents = async (basePdf: BasePdf): Promise<BasePdf> => {
    ...
    try {
      ...
      console.log(`Ensuring content streams for ${pageCount} pages`);
      ...
      <div className="w-72 border-r border-gray-200 bg-gray-50 flex flex-col p-3 space-y-3">
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-1">
          {pagesMeta?.map((page, index) => (
            <div
              key={page.id}
              className="p-2 border rounded bg-white shadow hover:shadow-md flex flex-col"
            >
              {/* Thumbnail container */}
              <div className="w-full mb-2 flex justify-center bg-gray-50 border-b border-gray-200 p-1">
                {page.thumbnailSrc ? (
                  <img 
                  ...
                  <div className="h-32 w-full flex items-center justify-center text-gray-400 bg-gray-50">
                    {template.basePdf === BLANK_PDF ? (
                      <span className="text-sm text-center">Blank Page</span>
                    ) : (
                      <span className="text-sm text-center">No Preview Available</span>
                    )}
                  </div>
                )}
              </div>
